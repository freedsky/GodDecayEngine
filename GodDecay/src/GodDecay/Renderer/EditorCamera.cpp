...
namespace GodDecay
{
	...
	{
		if (Input::IsKeyPressed(GODDECAY_KEY_LEFT_ALT))
		{
			//获取当前鼠标位置和初始化位置的偏移量是多少
			const glm::vec2& mouse{ Input::GetMouseX(), Input::GetMouseY() };
			glm::vec2 delta = (mouse - m_InitialMousePosition) * 0.003f;
			m_InitialMousePosition = mouse;

			//鼠标中键？左键控制旋转，右键控制zoom缩放
			if (Input::IsMouseButtonPressed(GODDECAY_MOUSE_BUTTON_MIDDLE))
				MousePan(delta);
			else if (Input::IsMouseButtonPressed(GODDECAY_MOUSE_BUTTON_LEFT))
				MouseRotate(delta);
			else if (Input::IsMouseButtonPressed(GODDECAY_MOUSE_BUTTON_RIGHT))
				MouseZoom(delta.y);
		}
